...
if __name__ == "__main__":
    ...
    drug_emb_df = pd.read_csv(
        "/ml_storage/jiaxin/tahoe_drug_unimol_embeddings_v1.csv"
    )
    drug_name_2_emb = {}
    ...
    with np.load(
        "/ml_storage/jiaxin/catch_up/best_performance_ae/benchmark_without_ae/embedding/decoded_raw_predict/1767971613.6208487_residual_swiglu_decoded.npz"
    ) as data:
        gene_log1p_mean_preds = data["decoded_means"].astype(np.float32)

    gene_log1p_mean_preds = torch.from_numpy(gene_log1p_mean_preds)
    ...
    class PairedGaussianDataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            ...
            return (
                torch.as_tensor(gene_log1p_mean_c, dtype=torch.float32),
                torch.as_tensor(gene_log1p_mean_p, dtype=torch.float32),
                gene_log1p_mean_preds[idx].to(torch.float32),
            )
    ...
    train_idx, val_idx, test_idx = get_split_dataset_indices()
    test_dataset = torch.utils.data.Subset(dataset, test_idx)

    max_test_idx = max(test_idx) if len(test_idx) > 0 else -1
    assert max_test_idx < len(gene_log1p_mean_preds), (
        f"pred array too short: max_test_idx={max_test_idx}, "
        f"len(pred)={len(gene_log1p_mean_preds)}. "
        "Check that your decoded preds were generated for the FULL paired dataset ordering."
    )

    print(f"Test subset size: {len(test_dataset)} / full paired: {len(dataset)}")

    # %%
    # Create dataloader
    dataloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=512, shuffle=False, num_workers=4, pin_memory=False
    )

    # %%
    # Compute metrics
    metrics = compute_perturbation_metrics_batched(
        dataloader, top_k_list=[200, 1000], device="cpu", verbose=True
    )
